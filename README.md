# 👨‍💻 Peter Dur Iliomo
### Spécialiste en Sécurité Informatique | Analyste Cybersécurité
🔐 Protection des systèmes | Sécurisation des données | Défense des réseaux

---

<details>
  <summary>🧠 Résumé Professionnel</summary>

Spécialiste en sécurité informatique basé à Kinshasa, RDC, avec expertise en cybersécurité, administration des systèmes, gestion des vulnérabilités et protection des bases de données. Certifié par Google et Cisco en Cybersécurité, Support IT et Gestion de projets.

</details>

<details>
  <summary>🔐 Compétences Clés</summary>

| Compétence | Niveau |
|------------|--------|
| Évaluation des vulnérabilités (OpenVAS, Nmap) | ⭐⭐⭐⭐ |
| Analyse du trafic réseau (Wireshark) | ⭐⭐⭐⭐ |
| Renforcement des systèmes Windows & Linux | ⭐⭐⭐⭐⭐ |
| Sécurité des bases de données MySQL | ⭐⭐⭐⭐ |
| Authentification sécurisée (JWT – Spring Boot) | ⭐⭐⭐⭐ |
| Surveillance des incidents & mitigation des risques | ⭐⭐⭐⭐⭐ |

</details>


- **Systèmes d'exploitation :** Linux, Windows Server  
- **Outils de sécurité :** Wireshark, OpenVAS, Nmap, Metasploit-framework  
- **Technologies :** MySQL, Java / Spring Boot, Git  

</details>

<details>

| Certificat | Année | Lien PDF |
|------------|-------|----------|
| Google Cybersecurity Professional Certificate | 2026 | [PDF](Google_Cybersecurity.pdf) |
| Google IT Support Professional Certificate | 2026 | [PDF](Google_IT_Support.pdf) |
| Gestion de Projet Google | 2026 | [PDF](Gestion_Projet.pdf) |
| Introduction à la Cybersécurité Cisco | 2025 | [PDF](Cisco_Security.pdf) |
| Hacker Éthique Cisco | 2026 | [PDF](Cisco_Ethical.pdf) |
| Introduction à l'IoT Cisco | 2025 | [PDF](Cisco_IoT.pdf) |
| Foundational Cybersecurity IBM | 2026 | [PDF](IBM_Cybersecurity.pdf) |

> 💡 Remplace les PDF par tes fichiers exacts dans le même dossier que README.md.

</details>

<details>
<summary>🚀 Projets & Labs</summary>

<details>
  <summary>🚀 Projets & Labs</summary>

| Projet | Description | Lien PDF |
|--------|------------|----------|
| 🔎 Laboratoire de Gestion des Vulnérabilités | Analyse et renforcement des systèmes vulnérables | [Lab1](Lab1.pdf) |
| 📡 Laboratoire d'Analyse du Trafic Réseau | Capture et analyse de paquets réseau | [Lab2](Lab2.pdf) |
| 🛡 Système de Gestion de Rendez-vous Médicaux Sécurisé (JWT) | Authentification et sécurité de données sensibles | [Project](Project.pdf) |
| 🛢 Projet de Renforcement de la Sécurité MySQL | Hardening de base de données et gestion des accès | [MySQL](MySQL.pdf) |

</details>

<details>
  <summary>📄 Documents & Téléchargements</summary>

| Document | Lien |
|----------|------|
| CV | [Télécharger CV](CV.pdf) |
| Certificats | [Voir Certificats](certificates.pdf) |
| Attestations | [Voir Attestations](attestations.pdf) |

</details>

<details>
  <summary>📫 Contact</summary>

- ✉ Email : [iliomopeter@gmail.com](mailto:iliomopeter@gmail.com), [peteriliomo@proton.me](mailto:peteriliomo@proton.me)  
- 🔗 LinkedIn : [Peter Iliomo](your_linkedin_url)

</details>

---

© 2026 Peter Dur Iliomo. Tous droits réservés.
